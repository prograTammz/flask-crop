...
def crop():
    if request.method == 'POST':
        ...
        file.save(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'images', file.filename))
        model()
        image = actions()
        files = glob.glob(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'images', '*'))
        for f in files:
            os.remove(f)        
        files = glob.glob(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'results', '*'))
        for f in files:
            os.remove(f)     
        data = {'image': image}
        return jsonify(data)
...
def actions():
    ...
    for name in names:
        # BACKGROUND REMOVAL
        file_name = name + '.png'
        output = Image.open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'results', file_name))
        ...
        file_name = name+'.jpg'
        input = Image.open(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'images', file_name))
